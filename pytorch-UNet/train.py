...
if __name__ == '__main__':
    ...
    # net = UNetcbam(num_classes).to(device)
    net = UNet1(n_classes=num_classes).to(device)

    # net = UNetCBAM(n_channels=3, n_classes=num_classes).to(device)
    ...
    while epoch < 200:
        for i, (image, segment_image) in enumerate(tqdm.tqdm(data_loader)):
            ...
            # image, segment_image = image.to(device), segment_image.squeeze(0).to(device)
            out_image = net(image)
